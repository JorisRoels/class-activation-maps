...
class UNet2D(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=2, feature_maps=64, levels=4, group_norm=False, pretrain_unsupervised=False):
        ...
        skip_connections = not pretrain_unsupervised
    ...
    def train_unsupervised_epoch(self, loader, loss_fn, optimizer, epoch, print_stats=1, writer=None, write_images=False):
    ...
    def test_unsupervised_epoch(self, loader, loss_fn, epoch, writer=None, write_images=False):
    ...
    # trains the network for one epoch
    def train_supervised_epoch(self, loader, loss_fn, optimizer, epoch, print_stats=1, writer=None, write_images=False):
    ...
    # tests the network over one epoch
    def test_supervised_epoch(self, loader, loss_fn, epoch, writer=None, write_images=False):
        ...
        if self.pretrain_unsupervised:
            ...
            for epoch in range(epochs):
                ...
                if epoch % test_freq == 0:
                    test_loss = self.test_unsupervised_epoch(loader=test_loader, loss_fn=loss_fn, epoch=epoch, writer=writer, write_images=True)
            ...
            for epoch in range(epochs):
                ...
                # test the model for one epoch is necessary
                if epoch % test_freq == 0:
                    test_loss = self.test_supervised_epoch(loader=test_loader, loss_fn=loss_fn, epoch=epoch, writer=writer, write_images=True)
